...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        //When
        double test = project.getTaskLists().stream()
                .filter(taskList -> taskList.getName().equals("In progress"))
                .flatMap(n -> n.getTasks().stream()).filter(n -> n.getDeadline().isAfter(LocalDate.now()))
                .map(t -> (int) ChronoUnit.DAYS.between(t.getCreated(), LocalDate.now()))
                .collect(Collectors.averagingDouble(n -> n));

        //Then
        Assert.assertEquals(5.0, test, 0.001);
    }
